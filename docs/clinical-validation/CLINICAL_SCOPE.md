...
- **(Project Vision Sprint 2) A second, separate real computer-vision candidate pipeline (Project Lens) exists alongside the YOLO placeholder above** — trained only from governed LCID images with ACTIVE Ground Truth, served through `app.services.ml.live_inference_adapter.predict()`, and reported to the frontend as "Candidate Model — Human Decision Required." It is registered `candidate_stage = "Experimental"` (trained on one declared synthetic-experimental run — this environment holds zero real facility ACTIVE Ground Truth today) and reports `not_promoted`/`unavailable` for every real inspection. It is **not clinically validated** and does not change the clinical scope stated in this document; see `docs/model-development/MODEL_CARD_REAL_V1.md` and `docs/model-development/REAL_MODEL_CURRENT_STATE_TRACE.md`.
