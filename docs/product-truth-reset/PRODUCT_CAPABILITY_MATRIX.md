...
## 12. Project Lens — First Real Computer-Vision Model & Live Inference Integration

| Field | Value |
|---|---|
| **Capability** | The first genuinely trained (not deterministic-placeholder) LumenAI vision model, plus a real live-inference adapter and honest feature-based baseline comparator |
| **Owner** | Backend: `app/services/ml/training_eligibility_service.py`, `lens_training_pipeline.py`, `lens_calibration.py`, `lens_model_registration.py`, `live_inference_adapter.py`, `image_similarity_service.py`, `experimental_dataset_generator.py`; additive integration in `app/services/baseline_comparison_scoring_service.py`/`app/routes/inspections.py` |
| **Status** | **Experimental** |
| **Implementation evidence** | A real, hierarchical (Stage B abnormality / Stage C category) one-vs-rest logistic-regression pipeline over real Pillow-computed features, trained via a genuinely real leakage-safe split (`dataset_split.split_dataset`, now correctly grouped by real barcode/UDI-backed Digital Twin identity rather than an "untracked" placeholder — a real bug found and fixed this sprint), real calibration (temperature scaling, new this sprint, layered on the pre-existing `evaluation.calibration_report()`), real error analysis, and a real, checksummed `ModelRegistryEntry` artifact. A real live inference adapter (`live_inference_adapter.predict()`) loads this artifact, decodes real image bytes, and returns the Section 19 result contract — wired additively into `analyze_inspection()` via a brand-new `live_model_result` key that never alters any pre-existing key (verified: the full 3614-test regression suite passes unchanged). A real, first-stage perceptual-hash (aHash) baseline comparator (`image_similarity_service.py`) replaces the prior complete absence of any pixel-level comparison anywhere in this codebase |
| **Tests** | `backend/tests/test_project_lens.py` (13 tests) — real image fixtures throughout, covering same-image/different-image comparison, baseline availability/compatibility gating, real bytes reaching the adapter and changing its prediction, safe unavailable states, unsupported-category exclusion, abstention (low confidence and insufficient quality), model ID/version persistence and prior-run immutability, cross-tenant isolation, and additive-only integration into `analyze_inspection()` |
| **Production availability** | The pipeline is live and real, but **this sprint's own registered model is trained exclusively on synthetic images** (one declared experimental run — see `docs/model-development/FIRST_MODEL_SCOPE.md`) — this environment has zero real facility ACTIVE Ground Truth today. The live adapter therefore reports `not_promoted`/`unavailable` for every real inspection today, by design: `lens_model_registration.register_lens_model()` never marks a synthetic-experimental-data model `"Candidate"`, and the adapter requires at least `"Candidate"` before serving a live prediction |
