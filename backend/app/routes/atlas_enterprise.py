...
from app.models.atlas_enterprise import ENTERPRISE_ROLES, ROLE_SCOPES
...
def get_enterprise_dashboard(
    system_id: str, db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
...
def post_refresh_enterprise_watchlist(
    system_id: str, db: Session = Depends(get_db), current_user=Depends(require_roles(*_LEADERSHIP_ROLES)),
):
...
def post_resolve_enterprise_watchlist_entry(
    system_id: str, entry_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_LEADERSHIP_ROLES)),
):
...
def get_facility_intelligence(
    system_id: str, facility_id: str, db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
...
def get_shared_knowledge_article(
    system_id: str, article_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
...
def post_retract_shared_knowledge_article(
    system_id: str, article_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_LEADERSHIP_ROLES)),
):
...
def post_acknowledge_enterprise_alert(
    system_id: str, alert_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
...
def post_resolve_enterprise_alert(
    system_id: str, alert_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_LEADERSHIP_ROLES)),
):
...
def post_generate_executive_report(
    system_id: str, payload: dict, db: Session = Depends(get_db), current_user=Depends(require_roles(*_LEADERSHIP_ROLES)),
):
    try:
...
def get_executive_reports(
    system_id: str, audience: str = Query(""), db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
...
def get_executive_report_csv(
    system_id: str, report_id: int, db: Session = Depends(get_db), current_user=Depends(require_roles(*_ALL_ROLES)),
):
