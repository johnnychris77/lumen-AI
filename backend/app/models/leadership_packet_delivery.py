...
class LeadershipPacketDelivery(Base):
    __tablename__ = "leadership_packet_deliveries"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    tenant_id: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    tenant_name: Mapped[str] = mapped_column(String(255), nullable=False)
    schedule_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    briefing_id: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    packet_id: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
